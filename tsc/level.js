...
function* generate() {
    ...
    yield newbox;
    function* repeat(n, f) { for (var i = 0; i < n; i++)
        yield newbox = f(newbox); }
    ...
    for (var i = 0; i < 20; i++) {
        ...
        switch (newdir) {
            case RIGHT:
                ...
                yield* repeat(randi(3, 6), cur => linkRight(cur, randi(1, 5), randi(1, 4), randi(1, 5)));
                break;
            case UPRIGHT:
                yield* repeat(4, linkUpr);
                break;
            case UPLEFT:
                if (prevdir == RIGHT)
                    continue;
                newbox.navi = "upl";
                yield* repeat(randi(1, 3), linkUpl);
                break;
            case UP:
                yield* repeat(randi(2, 6), randarr([linkUpl, linkUpr]));
                break;
            case DROP:
                if (prevdir == UPLEFT || prevdir == UP)
                    continue;
                newbox.navi = "drop";
                yield* repeat(randi(1, 3), drop);
                break;
            default: break;
        }
        curdir = newdir;
...
class Level {
    ...
    *doodads() {
        ...
        for (var box of this.boxes) {
            if (box.navi == "upl")
                yield ['signLeft', new Point(box.x + box.w / 2, box.t + 0.5)];
            if (box.navi == "drop")
                yield ['boxItem', new Point(box.r - 0.6, box.t - 0.6)];
